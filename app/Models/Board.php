...
class Board extends Model
{
     ...
     */
    public function statusCollections() { return $this->hasMany(StatusCollection::class); }

    /**
     * Grab all status's within related collections
     */
    public function statuses() { return $this->hasManyThrough(Status::class, StatusCollection::class); }

    /**
     * @param $optionsArray array of key => values that serve to fill in data object Model: Section
     */
    public function createSection($optionsArray) { $this->sections()->create($optionsArray); }

    /**
     * @param $optionsArray array of key => values that serve to fill in data object Model: Lead
     */
    public function createLead($optionsArray) { $this->leads()->create($optionsArray); }
}
